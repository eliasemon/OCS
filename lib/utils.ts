...
export function buildInstallUrl(appIds: string[], base = process.env.NEXT_PUBLIC_APP_URL || "https://installora-beta.vercel.app"): string {
  return `${base}/api/install.ps1?apps=${appIds.join(",")}`
}

export function buildPowerShellCommand(appIds: string[], base = process.env.NEXT_PUBLIC_APP_URL || "https://installora-beta.vercel.app"): string {
  const url = buildInstallUrl(appIds, base)
  return `powershell -ExecutionPolicy Bypass -c "irm ${url} | iex"`
}

export function buildCmdCommand(appIds: string[], base = process.env.NEXT_PUBLIC_APP_URL || "https://installora-beta.vercel.app"): string {
  const url = buildInstallUrl(appIds, base)
  return `powershell -ExecutionPolicy Bypass -Command "irm ${url} | iex"`
}

export function buildShareUrl(appIds: string[], base = process.env.NEXT_PUBLIC_APP_URL || "https://installora-beta.vercel.app"): string {
