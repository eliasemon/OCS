...
function Write-Banner {
  ...
  Write-Host "  ╚███╔███╔╝██║██║ ╚████║███████║███████╗   ██║   ╚██████╔╝██║     " -ForegroundColor $c
  Write-Host "   ╚══╝╚══╝ ╚═╝╚═╝  ╚═══╝╚══════╝╚══════╝   ╚═╝    ╚═════╝ ╚═╝    " -ForegroundColor $c
  Write-Host "  Automated Windows Package Installer — winsetup.app" -ForegroundColor DarkGray
  Write-Host ""
}
...
function Install-WingetIfMissing {
  Write-Host "[WinSetup] winget not found. Installing..." -ForegroundColor Yellow
  try {
    ...
    Write-Host "[WinSetup] winget installed." -ForegroundColor Green
  } catch {
    Write-Error "[WinSetup] Could not install winget: $_"; exit 1
  }
}
...
$ver = winget --version
Write-Host "[WinSetup] winget $ver" -ForegroundColor DarkGray
Write-Host "[WinSetup] Installing $($packages.Count) package(s)..." -ForegroundColor Cyan
...
if ($r.failed.Count -gt 0) {
  ...
  $fl = $r.failed -join ","
  Write-Host "  powershell -c \"irm https://winsetup.app/api/install.ps1?apps=$fl | iex\"" -ForegroundColor DarkGray
}
Write-Host ""
...
export function buildCliScript(): string {
  return `#Requires -Version 5.1
<# WinSetup CLI — Interactive Terminal Installer #>

$ErrorActionPreference = "Continue"
$baseUrl = "https://winsetup.app"

function Write-Banner {
  Write-Host ""
  Write-Host "  WinSetup CLI — Interactive Installer" -ForegroundColor Cyan
  Write-Host "  winsetup.app" -ForegroundColor DarkGray
  Write-Host ""
}
